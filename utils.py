...
def rot(vectors, deg):
    R_M = [[np.cos(deg), -np.sin(deg)], [np.sin(deg), np.cos(deg)]]
...
class FishEyeImage:
    def __init__(self, img_path, raw_path, f=14.6, k=-0.19, dpi=600):
        ...
        with rawpy.imread(raw_path) as rawfile:
            raw = rawfile.postprocess(gamma=(1, 1), no_auto_bright=True, output_bps=16)
        self.img = Image.open(img_path)
        with exiftool.ExifToolHelper() as et:
            self.iso = et.get_metadata(raw_path)[0]["EXIF:ISO"]

        self.raw = np.array(raw)[:, :, 1]
        self.raw[self.raw < 60000] = self.raw[self.raw < 60000] // (self.iso / 1600)
    ...
    def constellation(self, fn="test.jpg", cons_file_path=cons_file_path):
        ...
        for i in range(cons_lines.shape[0]):
            ...
            if angular_separation1 < 0.4 * np.pi or angular_separation1 < 0.4 * np.pi:
                if x2 < x1:
                    x1, x2 = x2, x1
                    y1, y2 = y2, y1
                k = (y2 - y1) / (x2 - x1)
